...
from self_ood.nn.encoder import EncoderArchitecture, encoder
from self_ood.nn.blocks import MLP
from self_ood.nn.functional import entropy, eval_mode


class SelfOOD(pl.LightningModule):
    def __init__(
            self,
            encoder_architecture: EncoderArchitecture,
            dropout_rate: float = 0.2,
            drop_channel_rate: float = 0.2,
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.trainer.world_size > 1:
            probas_1 = self.all_gather(probas_1, sync_grads=True).flatten(0, -2)  # (world_size, batch_size, num_prototypes)
            probas_2 = self.all_gather(probas_2, sync_grads=True).flatten(0, -2)
        memax_reg_1 = math.log(self.num_prototypes) - entropy(probas_1.mean(dim=0), dim=-1)
        memax_reg_2 = math.log(self.num_prototypes) - entropy(probas_2.mean(dim=0), dim=-1)
        memax_reg = (memax_reg_1 + memax_reg_2) / 2
        self.log(f'train/memax_reg', memax_reg, on_epoch=True, sync_dist=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        with eval_mode(self, enable_dropout=True):
            ...
            (ood_scores['mean_msp_on_views'],
             ...
             ood_scores['bald_score_on_views']) = self.compute_ensemble_scores(ensemble_probas)
    ...
    def sinkhorn(self, targets: torch.Tensor) -> torch.Tensor:
        gathered_targets = self.all_gather(targets).flatten(0, -2)
        num_targets, num_prototypes = gathered_targets.shape
        targets = targets / gathered_targets.sum()

        for _ in range(self.num_sinkhorn_iters):
            targets /= self.all_gather(targets).flatten(0, -2).sum(dim=0)
    ...
    @staticmethod
    def compute_ensemble_scores(ensemble_probas: torch.Tensor) -> torch.Tensor:
